...
def calibrateArray(data , calib_mat):
    ...
    for i in np.arange(0, acc.shape[0]):
        ...
        gyro_calib[i, :] = np.transpose(np.matmul(gyro_mat, (np.transpose([gyro[i, :]])-np.matmul(calib_mat.K_ga, np.transpose([acc_calib[i, :]]))-calib_mat.b_g)))
...
def calibrateSample(data, calib_mat):
    ...
    gyro_calib = np.transpose(np.matmul(gyro_mat, (np.transpose([gyro])-np.matmul(calib_mat.K_ga, np.transpose([acc_calib]))-calib_mat.b_g)))
...
def computeCalibrationMatrix(X_p, X_a, Y_p, Y_a, Z_p, Z_a, Rot_X, Rot_Y, Rot_Z, fs):
    ...
    for i in np.arange(0, gyroRot_X.shape[0]):
        gyroRot_x_corrected[i, :] = np.transpose(np.transpose([gyroRot_X[i, :]])-b_g-np.matmul(K_ga, np.transpose([accRot_X_calib[i, :]])))

    for i in np.arange(0,gyroRot_Y.shape[0]):
        gyroRot_y_corrected[i, :] = np.transpose(np.transpose([gyroRot_Y[i, :]])-b_g-np.matmul(K_ga, np.transpose([accRot_Y_calib[i, :]])))

    for i in np.arange(0, gyroRot_Z.shape[0]):
        gyroRot_z_corrected[i, :]=np.transpose(np.transpose([gyroRot_Z[i, :]])-b_g-np.matmul(K_ga, np.transpose([accRot_Z_calib[i, :]])))
...
def reverseCalibration(acc, gyro, calib_mat):
    ...
    :param K_a: Scaling matrix acceleration (numpy ndarray)
    :param R_a: Rotation matrix acceleration (numpy ndarray)
    :param b_a: bias vector acceleration (numpy ndarray)
    :param K_g: Scaling matrix gyroscope (numpy ndarray)
    :param R_g: Rotation matrix gyroscope (numpy ndarray)
    :param K_ga: Effect matrix of acceleration on gyroscope (numpy ndarray)
    :param b_g: bias vector gyroscope (numpy ndarray)
    :return: calibrated acceleration, calibrated gyroscope (numpy ndarray)
    '''
    ...
    for i in np.arange(0, acc.shape[0]):
        ...
        gyro_reverse[i, :]=np.transpose(np.matmul(gyro_mat, np.transpose(gyro[i, :])))+calib_mat.b_g+np.transpose(np.matmul(calib_mat.K_ga,np.transpose([acc[i,:]])))
